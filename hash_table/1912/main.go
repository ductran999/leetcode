...
func (ms *MovieRentingSystem) Search(movie int) []int {
	movies, ok := ms.unrentedMovies[movie]
	if !ok {
		return nil
	}
	result := make([]int, 0, 5)
...
func (ms *MovieRentingSystem) Rent(shop int, movie int) {
	...
	rentingMovie := ms.indexingKeyOffset[key.String()]
	if rentingMovie == nil {
		return
	}

	heap.Remove(ms.unrentedMovies[movie], rentingMovie.Offset)
	...
	ms.rentedMovies[key.String()] = rentedMovie
	heap.Push(ms.rentedMovieHeap, rentedMovie)
}

func (ms *MovieRentingSystem) Drop(shop int, movie int) {

	key := IndexKey{
		Shop:  shop,
		Movie: movie,
	}
	m, ok := ms.rentedMovies[key.String()]
	if !ok {
		return
	}
	...
	ms.indexingKeyOffset[key.String()] = rentingMovie

	heap.Push(ms.unrentedMovies[movie], rentingMovie)

	heap.Remove(ms.rentedMovieHeap, m.Offset)

	delete(ms.rentedMovies, key.String())
}

func (ms *MovieRentingSystem) Report() [][]int {
	result := make([][]int, 0, 5)
	historicals := make([]*RentingMovie, 0, 5)
	for ms.rentedMovieHeap.Len() > 0 && len(historicals) < 5 {
		item := heap.Pop(ms.rentedMovieHeap)
		m := item.(*RentingMovie)
	...
	for _, item := range historicals {
		heap.Push(ms.rentedMovieHeap, item)
	}
	return result
}
